    ...
    try {
        ...
        if (req.file) {
            ...
            const result = await new Promise((resolve, reject) => {
                cloudinary.uploader.upload_stream({ folder: "pet_profiles", format: "jpg" }, (error, uploadedFile) => {
                    if (error)
                        reject(error);
                    else
...
export const getMedications = async (req, res) => {
    try {
        ...
        if (updateMedicationsStatus.length) {
            await Medication.updateMany({ _id: { $in: updateMedicationsStatus.map(med => med._id) } }, { is_ongoing: false });
        }
...
export const addMedReminder = async (req, res) => {
    try {
        const userId = req.id;
        const { petId } = req.query;
        const { medId, frequency, starting_date, end_date, reminder_times, reminder_methods, repeat_reminder } = req.body;
...
export const updateMedReminder = async (req, res) => {
    try {
        ...
        const updatedScheduleReminder = await MedicationReminder.findByIdAndUpdate(id, {
            medication: medId,
            frequency,
            starting_date,
            end_date,
...
export const markGivenMedReminder = async (req, res) => {
    try {
        ...
        const result = await MedicationReminder.updateOne({ _id: id }, {
            $set: {
            ...
            for (let index = 0; index < (reminder.reminder_times || []).length; index++) {
                ...
                if (rt.last_notified) {
                    const lastNotifiedDay = moment(rt.last_notified)
                        .tz(userTz)
                        .startOf("day");
                    if (lastNotifiedDay.isSame(today))
                        continue;
                }
                const [hour, minute] = rt.time.split(":").map(Number);
                const reminderTimeToday = today.clone().set({
                ...
                if (diffMinutes <= remindBeforeMins &&
