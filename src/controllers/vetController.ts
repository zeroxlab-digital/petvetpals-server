...
import { LoginUserDTO, RegisterUserDTO, UpdateVetDTO } from "../types/user.types.js";
...
export const registerVet = async (req: Request<{}, {}, RegisterUserDTO>, res: Response) => {
    try {
        ...
        const vet_token = jwt.sign(tokenData, process.env.JWT_SECRET_KEY, { expiresIn: '60d' });
        res.status(200).cookie("vet_token", vet_token, cookieOptions).json({ success: true, message: "Vet registration successfull!" });
    } catch (error) {
        console.log(error)
...
export const loginVet = async (req: Request<{}, {}, LoginUserDTO>, res: Response) => {
    try {
        ...
        const vet_token = jwt.sign(tokenData, process.env.JWT_SECRET_KEY!, { expiresIn: '60d' });
        res.status(200).cookie("vet_token", vet_token, cookieOptions).json({ success: true, message: "Vet login successfull!" });
    } catch (error) {
        console.log(error);
        res.status(400).json({ message: "Internal server error!", error });
    }
...
export const updateVetProfile = async (req: Request<{}, {}, UpdateVetDTO>, res: Response) => {
    try {
        const vetId = req.id;
