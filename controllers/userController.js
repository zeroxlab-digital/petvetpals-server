...
export const userLogin = async (req, res) => {
    try {
        ...
        res.status(200).cookie("user_token", user_token, { maxAge: 1 * 24 * 60 * 60 * 1000, sameSite: "None", secure: process.env.NODE_ENV === "production" }).json({ success: 'true', message: "User login successfull!", userDetails })
    } catch (error) {
        console.log(error);
        res.status(400).json({ message: "Internal server error!", error });
    }
